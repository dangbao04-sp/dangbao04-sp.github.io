...
export const getChatbotResponse = async (prompt: string): Promise<string> => {
    try {
        // FIX: Corrected call to generateContent and response text extraction.
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: prompt, // Use `contents` property for the prompt
            config: {
                systemInstruction: systemInstruction,
            },
        });
        return response.text; // Access the `.text` property directly
    } catch (error) {
